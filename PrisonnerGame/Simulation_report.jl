using Distributions, DataFrames, Statistics, Plots, StatsPlots, StatsBase
...
###########################################
# Run Monte Carlo Simulation and Analysis
###########################################
# Settings:
...
df_results = run_monte_carlo(n_simulations, rounds_per_run)
...
# Plot Histograms
###########################################
# Histogram for overall US cumulative payoff.
histogram(df_results.us_total,
    bins=30,
    title="Histogram of Overall US Cumulative Payoff",
    xlabel="US Cumulative Payoff (Normalized Billion-Dollar Units)",
    ylabel="Frequency",
    legend=false)
savefig("us_total_histogram.png")

# Histogram for overall US average tariff rate.
histogram(df_results.overall_US_avg_tariff,
    bins=30,
    title="Histogram of Overall US Average Tariff Rate",
    xlabel="Overall US Average Tariff Rate (%)",
    ylabel="Frequency",
    legend=false)
savefig("us_avg_tariff_histogram.png")

# Histogram for overall US impact on GDP.
histogram(df_results.overall_US_impact,
    bins=30,
    title="Histogram of Overall US Impact on GDP (%)",
    xlabel="US Impact on GDP (%)",
    ylabel="Frequency",
    legend=false)
savefig("us_impact_histogram.png")

###########################################
# Plot Empirical Cumulative Distribution (S Curve)
###########################################
# CDF for overall US cumulative payoff.
plot(ecdf(df_results.us_total),
    title="CDF of Overall US Cumulative Payoff",
    xlabel="US Cumulative Payoff (Normalized Billion-Dollar Units)",
    ylabel="Cumulative Probability",
    legend=false)
savefig("us_total_cdf.png")

# CDF for overall US average tariff rate.
plot(ecdf(df_results.overall_US_avg_tariff),
    title="CDF of Overall US Average Tariff Rate",
    xlabel="Overall US Average Tariff Rate (%)",
    ylabel="Cumulative Probability",
    legend=false)
savefig("us_avg_tariff_cdf.png")

# CDF for overall US impact on GDP.
plot(ecdf(df_results.overall_US_impact),
    title="CDF of Overall US Impact on GDP (%)",
    xlabel="US Impact on GDP (%)",
    ylabel="Cumulative Probability",
    legend=false)
savefig("us_impact_cdf.png")
